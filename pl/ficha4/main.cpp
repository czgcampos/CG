...
float alfa=0.0f,beta=0.0f,radius=5.0f,camX,camY,camZ;
GLuint buffers[1];
float *vertexB=NULL;
int n=0;
...
void prepareCylinder(float radius,float height,int sides){
	n=sides*3+sides*6+sides*3;
	vertexB=new float[n*3];
	int i,j=0;
	...
	glBindBuffer(GL_ARRAY_BUFFER,buffers[0]);
	glBufferData(GL_ARRAY_BUFFER,n*3*sizeof(float),vertexB,GL_STATIC_DRAW);
}
